...
папке. HR-кабинет в `../frontend` подключён к API; источник данных — PostgreSQL.
`GET /api/v1/hr/workspace` возвращает данные только разрешённых HR подразделений.
HR-интерфейс работает в режиме просмотра, а предварительные рекомендации
рассчитывает в браузере без создания серверного снимка. Импорт доступен через
admin API или CLI. Мобильный UI сотрудника в `../frontend/mobile` использует
тот же API и PostgreSQL: профиль, навыки, карьерная карта, заявления и записи
загружаются через `/api/v1/me/*`; рекомендации рассчитываются на сервере.
...
# Для демонстрации: DEMO_AUTH=true, DEMO_SEED=true,
# собственный DEMO_PASSWORD (от 12 символов).
docker compose up --build -d --wait
```

Из корня репозитория можно выполнить `./start.ps1` либо
...
миграции и явно включённый seed. При `DEMO_SEED=true` bootstrap также импортирует
канонические данные из `../frontend/public/data` и выдаёт демонстрационному HR
доступ к их подразделениям. HR-кабинет: http://localhost:5173, логин `hr`.
Кабинет сотрудника: http://localhost:3000, логин `employee`.
Пароль обоих демонстрационных аккаунтов — значение `DEMO_PASSWORD` из `.env`.
...
Мобильный кабинет позволяет выбрать навык из серверного каталога и заявить
уровень с evidence, выбрать вакансию как карьерную цель, записаться на активность,
отправить результат на подтверждение и запросить оценку для повышения.
`GET /api/v1/me/completion-requests` возвращает с пагинацией только заявки
по записям текущего сотрудника. Отправка результата не начисляет навыки:
подтверждение выполняет независимый уполномоченный оценщик через API.
Повышение требует отдельного решения HR. Настройки интерфейса сохраняются
через `PATCH /api/v1/me/preferences`.
...
При Docker-сборке мобильного UI выполняются `npm test`, `npm run lint`
(проверка TypeScript) и `npm run build` в Node.js 24. Команды разработки
описаны в [README мобильного кабинета](../frontend/mobile/README.md).
...
Правило `/docs/` в корневом `.gitignore` исключает только корневую папку
документов; `backend/docs/` по-прежнему отслеживается Git.
